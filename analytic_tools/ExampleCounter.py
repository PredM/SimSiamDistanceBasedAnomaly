...
def main():
    ...
    # import datasets
    y_test_features = np.load(config.training_data_folder + "test_features_2_.npy")
    y_train_features = np.load(config.training_data_folder + "train_features_2_.npy")
    y_train = np.load(config.training_data_folder + "train_labels_2_.npy")  # labels of the training data
    y_test = np.load(config.training_data_folder + "test_labels_2_.npy")  # labels of the training data
    y_train_window_times = np.load(config.training_data_folder + "train_window_times_2_.npy")  # labels of the training data
    y_test_window_times = np.load(config.training_data_folder + "test_window_times_2_.npy")  # labels of the training data
    y_train_failure_times = np.load(config.training_data_folder + "train_failure_times_2_.npy")  # labels of the training data
    y_test_failure_times = np.load(config.training_data_folder + "test_failure_times_2_.npy")  # labels of the training data

    feature_names = np.load(config.training_data_folder + "feature_names_2_.npy")
    ...
    failure_Labes_to_remove_2 = ['txt15_i3_lightbarrier_failure_mode_1',
                               'txt16_pneumatic_leakage_failure_mode_1', 'txt18_pneumatic_leakage_failure_mode_3_faulty' ]

    failure_to_extract_from_train_to_test = ["2019-05-28 08:49:58", "2019-05-28 08:27:24", "2019-05-28 08:19:18", "2019-05-23 10:17:54", "2019-05-28 19:35:08",
                                             "2019-06-08 11:46:28", "2019-06-08 11:57:40", "2019-05-23 13:39:55", "2019-05-28 19:12:09", "2019-06-07 20:20:52",
                                             "2019-05-28 20:26:03", "2019-06-07 20:13:24", "2019-06-08 13:30:11","2019-05-28 15:31:22"]
    features_to_remove = ['a_15_c_x','a_15_c_y','a_15_c_z','a_18_1_x','a_18_1_y','a_18_1_z','hrs_gyr_x','hrs_gyr_y',
    ...
    y_train_features, feature_names_ = removeAttributes(features=y_train_features, feature_list_names= feature_names, features_to_remove_list=features_to_remove)
    y_test_features, feature_names = removeAttributes(features=y_test_features, feature_list_names= feature_names, features_to_remove_list=features_to_remove)
    ...
    print(" Before y_train_features shape: ", y_train_features.shape, "y_train_window_times shape: ", y_train_window_times.shape)
    ...
    print(" After: y_train_features shape: ", y_train_features.shape, "y_train_window_times shape: ", y_train_window_times.shape)
    ...
    y_test_features, y_test, y_test_window_times, y_test_failure_times = appendExtractedExamples(features=y_test_features, labels= y_test, window_times= y_test_window_times,
    ...
    ### Rename Labels Train ###
    y_train = np.char.replace(y_train, "txt15_failuremode_driveshaft_slippage_2", "txt15_conveyor_failuremode_driveshaft_slippage_failure")
    ...
    y_train = np.char.replace(y_train, "txt16_failuremode_conveyorbelt_broken_big_gear_tooth_diagnosis","txt16_conveyorbelt_big_gear_tooth_broken_failure")
    y_train = np.char.replace(y_train, "txt16_failuremode_conveyorbelt_broken_big_gear_tooth_diagnosis_2","txt16_conveyorbelt_big_gear_tooth_broken_failure")
    y_train = np.char.replace(y_train, "txt16_failuremode_conveyorbelt_broken_small_gear_tooth_diagnosis","txt16_conveyorbelt_small_gear_tooth_broken_failure")
    y_train = np.char.replace(y_train, "txt16_failuremode_driveshaft_slippage", "txt16_conveyor_failuremode_driveshaft_slippage_failure")
    y_train = np.char.replace(y_train, "txt16_failuremode_turntable_broken_bigh_gear_tooth_2", "txt16_turntable_big_gear_tooth_broken_failure")
    ...
    y_train = np.char.replace(y_train, "txt16_conveyorbelt_big_gear_tooth_broken_failure_2", "txt16_conveyorbelt_big_gear_tooth_broken_failure")
    y_train = np.char.replace(y_train, "txt18_pneumatic_leakage_failure_mode_1_2", "txt18_pneumatic_leakage_failure_mode_1")
    y_train = np.char.replace(y_train, "txt18_pneumatic_leakage_failure_mode_2_2","txt18_pneumatic_leakage_failure_mode_2")
    y_train = np.char.replace(y_train, "txt18_pneumatic_leakage_failure_mode_2_1","txt18_pneumatic_leakage_failure_mode_2_faulty")
    y_train = np.char.replace(y_train, "txt18_pneumatic_leakage_failure_mode_3_1","txt18_pneumatic_leakage_failure_mode_3_faulty")
    y_train = np.char.replace(y_train, "txt17_pneumatic_leakage_failure_mode_1_1", "txt17_pneumatic_leakage_failure_mode_1_faulty")
    y_train = np.char.replace(y_train, "txt17_pneumatic_leakage_failure_mode_1_2", "txt17_pneumatic_leakage_failure_mode_1")
    y_train = np.char.replace(y_train, "txt17_failuremode_workingstation_transport_without_Workpiece", "txt17_workingstation_transport_failuremode_wout_workpiece")
    y_train = np.char.replace(y_train, "txt18_TransportWithoutWorkpiece_diagnosis", "txt18_transport_failuremode_wout_workpiece")
    # General
    ...
    ### Rename Labels Test ###
    y_test = np.char.replace(y_test, "txt15_failuremode_driveshaft_slippage_2", "txt15_conveyor_failuremode_driveshaft_slippage_failure")
    ...
    y_test = np.char.replace(y_test, "txt16_failuremode_conveyorbelt_broken_big_gear_tooth_diagnosis","txt16_conveyorbelt_big_gear_tooth_broken_failure")
    y_test = np.char.replace(y_test, "txt16_failuremode_conveyorbelt_broken_big_gear_tooth_diagnosis_2","txt16_conveyorbelt_big_gear_tooth_broken_failure")
    y_test = np.char.replace(y_test, "txt16_failuremode_conveyorbelt_broken_small_gear_tooth_diagnosis","txt16_conveyorbelt_small_gear_tooth_broken_failure")
    y_test = np.char.replace(y_test, "txt16_failuremode_driveshaft_slippage", "txt16_conveyor_failuremode_driveshaft_slippage_failure")
    y_test = np.char.replace(y_test, "txt16_failuremode_turntable_broken_bigh_gear_tooth_2", "txt16_turntable_big_gear_tooth_broken_failure")
    ...
    y_test = np.char.replace(y_test, "txt16_conveyorbelt_big_gear_tooth_broken_failure_2", "txt16_conveyorbelt_big_gear_tooth_broken_failure")
    y_test = np.char.replace(y_test, "txt18_pneumatic_leakage_failure_mode_1_2", "txt18_pneumatic_leakage_failure_mode_1")
    y_test = np.char.replace(y_test, "txt18_pneumatic_leakage_failure_mode_2_2","txt18_pneumatic_leakage_failure_mode_2")
    y_test = np.char.replace(y_test, "txt18_pneumatic_leakage_failure_mode_2_1","txt18_pneumatic_leakage_failure_mode_2_faulty")
    y_test = np.char.replace(y_test, "txt18_pneumatic_leakage_failure_mode_3_1","txt18_pneumatic_leakage_failure_mode_3_faulty")
    y_test = np.char.replace(y_test, "txt17_pneumatic_leakage_failure_mode_1_1", "txt17_pneumatic_leakage_failure_mode_1_faulty")
    y_test = np.char.replace(y_test, "txt17_pneumatic_leakage_failure_mode_1_2", "txt17_pneumatic_leakage_failure_mode_1")
    y_test = np.char.replace(y_test, "txt17_failuremode_workingstation_transport_without_Workpiece", "txt17_workingstation_transport_failuremode_wout_workpiece")
    y_test = np.char.replace(y_test, "txt18_TransportWithoutWorkpiece_diagnosis", "txt18_transport_failuremode_wout_workpiece")
    y_test = np.char.replace(y_test, "txt18_TransportWithoutWorkpiece_diagnosis", "txt18_transport_failuremode_wout_workpiece")
    # General
...
def appendExtractedExamples(features, labels, window_times, failure_times,features_ext, labels_ext, window_times_ext, failure_times_ext):
